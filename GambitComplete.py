...
color_dark_wall = (139, 35, 35)
color_light_wall = (83, 134, 139)
color_dark_ground = (0,100,0)
color_light_ground = (61, 145, 64)
...
def create_room(room):
    global my_map
    #go through the tiles in the rectangle and make them passable changing 
...
def place_special_rooms():  #TODO: Redo the dungeon generation to allow for special rooms, or ensure v_tun and h_tun will not intersect special room
	...
	#add princess in fancy room
	(new_x, new_y) = special_room.center()
	princess = GameObject(new_x, new_y, '&', 'the Princess', colors.light_pink, blocks=True)
	objects.append(princess)
	...
	for x in range(9):
		decy -= 2
		decoration = GameObject(decx, decy, '%', 'Fountain', colors.lighter_blue, blocks=False)
...
def place_objects(room):
	...
	num_monsters = randint(MIN_ROOM_MONSTERS, MAX_ROOM_MONSTERS)
	...
	for i in range(num_monsters):
		...
		if not is_blocked(x, y):
			if randint(0, 100) < 80:  #80% chance of getting a guard 
                #create a guard`
				...
				monster = GameObject(x, y, 'g', 'a guard', colors.black, blocks=True, fighter=fighter_component, ai=ai_component)
			else:
                #create a Captain of the Guards
				fighter_component = Fighter(hp=16, defense=1, power=4, 
											death_function=monster_death)
                
				ai_component = BasicMonster()
				monster = GameObject(x, y, 'G', 'a Captain of the Guard', colors.black, 
	...
	for i in range(num_items):
		...
		if not is_blocked(x, y):
			...
			if dice < 70:
				...
				item = GameObject(x, y, '0', 'scroll of confusion', 
                                  colors.light_yellow, item=item_component)
...
def render_bar(x, y, total_width, name, value, maximum, bar_color, back_color):
    ...
    panel.draw_str(x_centered, y, text, fg=colors.black, bg=None)
...
def cast_lightning():
    ...
    if monster is None:  #no enemy found within maximum range
        message('No enemy is close enough to throw the horseshow at.', colors.red)
        return 'cancelled'
 
    #zap it!
    message('Your lucky horseshoe strikes ' + monster.name + ' with a quiet ' +
            'thump! The damage is ' + str(LIGHTNING_DAMAGE) + ' hit points.', 
            colors.light_blue)
...
def cast_confuse():
    ...
    message('Left-click an enemy to flick the lucky coin at it, or right-click to cancel.', 
            colors.light_cyan)
    ...
    message('The ' + monster.name + ' eyes the coin that struck him ' + 
			'square in in the middle of his forehead. ' + 
			'He is distracted: Run!', colors.light_green)
...
def new_game():
    ...
    message('''Zheel is unaware that he is too drunk to sneak. 
				You might want to help him.
				Press 'g' to gather items. 
				Press 'i' to see inventory.
				Press ALT + ENTER for fullscreen''', colors.light_green)
def play_game():
...
def prologue():
	#img = image_load('prologue.png')
	#con.clear()
	#img.blit(con, 80, 47, 0, 1, 1, 0)
	root.clear()
	title = "Zheel was out drinking, when he was dared to steal a lock of the Princess\' hair. Now he is sneaking into the castle, searching for his prize...."
...
def epilogue():
	story = ["As Zheel stumbled into the chamber, he was stopped short at the sight of a girl," 
			"\"Why are you here?\" The girl asked," 
			"\"For you of course.\" smiled Zheel," 
			"Though she was a homely lass, Zheel began to shower her with compliments, and before long, they had spent the night together,"
			"But as the sun rose, Zheel recognized her face from the countless coins he'd handled...,"
			"and suddenly he felt ill, for he knew the fate of thieves caught in the palace,"
			"But because this thief was kind and bold, he was sent from the palace laden with gold," 
			"And so Zheel lost his bet and thieved no more. Instead he opened the Empresses Lock, the finest tavern in Greel"]
	offset = 4
	root.clear()
	for i in story:
		center = (SCREEN_WIDTH - len(i)) //2
		root.draw_str(center, SCREEN_HEIGHT//2-offset, i, bg=None, fg=colors.light_yellow)
		offset -= 2
	root.blit(con, 25, 40)
	tdl.flush()
	while not tdl.event.is_window_closed():
...
def main_menu():
    img = image_load('Paris-Catacombs.png')
    ...
    while not tdl.event.is_window_closed():
        ...
        title = 'A PRUDENT THIEF'
        center = (SCREEN_WIDTH - len(title)) //2
        root.draw_str(center, SCREEN_HEIGHT//2-4, title, bg=None, fg=colors.light_yellow)
 
        title = 'Created By : Nicole Lawrence & Alex Hocker (& Jotaf)'
        center = (SCREEN_WIDTH - len(title)) //2
...
root = tdl.init(SCREEN_WIDTH, SCREEN_HEIGHT, title="Zheel - A Prudent Thief", 
                fullscreen=False)
